...
export default function Settings() {
  const { user, isDarkMode, userRole } = useApp();
  
  // ✅ FIX: All hooks MUST be called unconditionally at the top
  ...
  async function handleUpdateProfile(e: FormEvent) {
    ...
    try {
      ...
      // Sync profiles table for RBAC consistency
  ...
  // ✅ FIX: Conditional rendering AFTER hooks ensures React stability
  if (!user) {
    return (
      ...
      <div className={cardClass}>
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-5">
          <div>
            ...
            <input disabled value={user.email || ''} className={`${inputClass} opacity-60 cursor-not-allowed bg-gray-100 dark:bg-gray-800`} />
          ...
          <div>
            ...
            <div className={`px-4 py-2.5 rounded-xl border text-sm font-medium ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-300' : 'bg-gray-50 border-gray-200 text-gray-600'}`}>
              {userRole || 'N/A'}
            </div>
          </div>
